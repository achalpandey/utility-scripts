...
gateway() {
  ...
  local app_dir="" d="$PWD"
  while [ "$d" != "/" ]; do
    [ -d "$d/apps/vmc_architecture_mvp" ] && { app_dir="$d/apps/vmc_architecture_mvp"; break; }
    [ "$(basename "$d")" = "vmc_architecture_mvp" ] && { app_dir="$d"; break; }
    d=$(dirname "$d")
  done
  : "${app_dir:=$HOME/Sandbox/llm-gateway/main/apps/vmc_architecture_mvp}"
...
gcv() {
  ...
  case "$sub" in
    scp)
      ...
      local args=() a
      for a in "$@"; do
        case "$a" in :*) args+=("${vm}${a}") ;; *) args+=("$a") ;; esac
      done
